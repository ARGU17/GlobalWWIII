...
export const VERSION="5.4.1",SCHEMA=54;
...
export function createArchitecture({engine,config,workerUrl=null}){
  ...
  if(!Array.isArray(config.resources)||config.resources.length!==21||new Set(config.resources.map(item=>item.id)).size!==21)throw new Error("El catálogo v5.4.1 debe declarar exactamente 21 mercados diferenciados");
  ...
  function createInitialState(){const state=attach(legacy.create());engine.pushEvent?.(state,"system","Strategic Command v5.4.1 · 21 mercados visibles","La barra superior conecta en tiempo real producción, demanda, balance, inventarios y precios de los 21 mercados diferenciados.");return state}
...
async function loadConfig(base){const read=async name=>{const response=await fetch(new URL(`data/v54/${name}`,base));if(!response.ok)throw new Error(`No se pudo cargar ${name}`);return response.json()};const[owners,events,schema,manifest,resources]=await Promise.all([read("ownership.json"),read("event-types.json"),read("state.schema.json"),read("systems.json"),read("market-resources.json")]);return{owners,events,schema,manifest,resources}}
...
if(typeof window!=="undefined"&&typeof document!=="undefined"){
  try{
    ...
    const{bootLegacyUi}=await import("../ui/bootstrap.mjs?v=5.4.1-r1");await bootLegacyUi();
  }catch(error){
