...
const read=name=>JSON.parse(fs.readFileSync(path.join(root,"data/v54",name),"utf8")),config={owners:read("ownership.json"),events:read("event-types.json"),schema:read("state.schema.json"),manifest:read("systems.json")},architecture=createArchitecture({engine:globalThis.NEXUS_ECONOMY,config}),days=Math.max(0,Number(process.argv[2])||30),state=architecture.runHeadless(null,days),controlled=state.countries.find(x=>x.id===state.controlledCountryId);
