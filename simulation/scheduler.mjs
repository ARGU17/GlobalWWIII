...
export class SimulationScheduler{
  ...
  register(system){if(!system?.id||!system?.owner||!system?.frequency||typeof system.run!=="function")throw new Error("Sistema de simulación incompleto");if(this.systems.some(x=>x.id===system.id))throw new Error(`Sistema duplicado: ${system.id}`);this.systems.push({...system,order:Number(system.order)||100});this.systems.sort((a,b)=>a.order-b.order||a.id.localeCompare(b.id));return system}
