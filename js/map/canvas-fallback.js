...
window.NEXUS_CANVAS_MAP_ENGINE = (() => {
  ...
  function initialize(nextState,nextCallbacks={}){
    ...
    canvas=document.getElementById("strategicMapFallback")||document.getElementById("strategicMap");tooltip=document.getElementById("mapTooltip");
    if(!canvas||canvas.tagName!=="CANVAS")throw new Error("El fallback cartográfico requiere un canvas compatible.");
  ...
  async function loadSpainRegions(){
    const urls=["assets/maps/v6/spain_autonomous_regions.json","assets/maps/spain-autonomous-regions.topojson"];
  ...
  function getTile(z,x,y){const template=window.NEXUS_MAP_TILE_PROVIDER?.urlTemplate;if(!template)return null;const key=`${z}/${x}/${y}`;if(tileCache.has(key))return tileCache.get(key);if(tileCache.size>260){const first=tileCache.keys().next().value;tileCache.delete(first)}const img=new Image();img.decoding="async";img.onload=()=>{needsRender=true};img.onerror=()=>{img.failed=true};img.src=template.replace("{z}",z).replace("{x}",x).replace("{y}",y);tileCache.set(key,img);return img}
