...
window.NEXUS_UI = (() => {
  ...
  const fmt2 = n => new Intl.NumberFormat("es-ES", {minimumFractionDigits:2,maximumFractionDigits:2}).format(Number(n)||0);
  ...
  function renderNavigation() {
    ...
    const title=document.getElementById("mapTitle"),labels={political:"Soberanía y rutas comerciales",economy:"PIB y capacidad económica",military:"Unidades, movimientos y frentes militares",industry:"Instalaciones industriales y niveles",technology:"Capacidad tecnológica nacional",stability:"Estabilidad y cohesión territorial"};if(title)title.textContent=labels[state.mapLayer||"political"];
  }
  ...
  function renderInspector() {
    ...
    const sovereign=state.countries.filter(x=>x.sovereign!==false),rank=[...sovereign].sort((a,b)=>b.economy.gdp-a.economy.gdp).findIndex(x=>x.id===c.id)+1,totalGDP=sovereign.reduce((s,x)=>s+(x.economy.gdp||0),0),debtStock=(c.economy.gdp||0)*(c.economy.debtRatio||0)/100;
    ...
    box.innerHTML=`<div class="country-hero"><div class="country-flag">${c.flag}</div><div><h2>${esc(c.name)}</h2><p>${esc(c.government.regime)}</p></div></div>
      ...
      <div class="info-list">${info("Gobierno",ruling?.name||c.government.ideology)}${info("PIB mundial",rank>0?`#${rank} · ${fmt1(c.economy.gdp/Math.max(1,totalGDP)*100)}%`:"Territorio integrado")}${info("PIB per cápita",`${fmt0(c.economy.gdp*1000/Math.max(.1,c.economy.population))} €`)}${info("Deuda",`${pct(c.economy.debtRatio)} · ${money(debtStock)}`)}${info("Interés",pct(c.economy.interestRate||0))}${info("Inflación",pct(c.economy.inflation))}${info("Desempleo",pct(c.economy.unemployment))}${info("Preparación",pct(c.militaryReadiness))}</div>
  ...
  function renderEconomy() {
    ...
    return `${heading("Economía nacional","Fiscalidad, gasto, deuda, capacidad productiva, empleo y comercio")}
      ...
      <div class="economy-layout">
        <section class="card span-2"><div class="card-title"><h3>Presupuesto nacional</h3><span>Los cambios afectan tecnología, ejército, energía y crecimiento cada mes</span></div><div class="budget-grid">${Object.entries(c.budgets).map(([k,v])=>budgetSlider(k,v)).join("")}${taxSlider(c.economy.taxRate)}</div><div class="budget-impact">${info("I+D prevista",`${fmt1(c.researchMonthlyRate||E().calculateResearchRate?.(state,c)||0)} puntos/mes`)}${info("Tecnología mensual",`${(c.budgetImpact?.technology||0)>=0?"+":""}${fmt2(c.budgetImpact?.technology||0)}`)}${info("Capacidad militar",`${(c.budgetImpact?.military||0)>=0?"+":""}${fmt2(c.budgetImpact?.military||0)}`)}${info("Energía",`${(c.budgetImpact?.energy||0)>=0?"+":""}${fmt2(c.budgetImpact?.energy||0)}`)}</div></section>
        ...
        <section class="card"><div class="card-title"><h3>Gestión de deuda</h3><span>Menos intereses liberan tesorería a largo plazo</span></div>${info("Ratio de deuda",pct(c.economy.debtRatio))}${info("Stock estimado",money(c.economy.gdp*c.economy.debtRatio/100))}${info("Ahorro mensual",money(c.economy.debtInterestSavings||0))}<div class="debt-actions"><button data-action="payDebt" data-share="1">Amortizar 1%</button><button data-action="payDebt" data-share="5">Amortizar 5%</button><button data-action="payDebt" data-share="10">Amortizar 10%</button></div></section>
  ...
  function renderRegions() {
    ...
    let region=regions.find(r=>r.id===state.selectedRegionId)||regions[0];if(!state.selectedRegionId)state.selectedRegionId=region.id;
    ...
    return `${heading(`Territorios · ${c.flag} ${c.name}`,"Selecciona una región, inspecciona sus recursos y gestiona capacidad productiva",`<select data-region-select>${regions.map(r=>`<option value="${r.id}" ${r.id===region.id?"selected":""}>${esc(r.name)}</option>`).join("")}</select>`)}
      ...
      <div class="region-detail"><div class="kpi-grid four">${kpi("PIB",money(region.gdp||0),region.capital||"")}${kpi("Población",`${fmt2(region.population||0)} M`,`${fmt0(region.dynamicEconomy?.gdpPerCapita||0)} €/hab.`)}${kpi("Empleo industrial",fmt0(region.dynamicEconomy?.facilityJobs||0),"dinámico")}${kpi("Infraestructura",fmt1(region.infra||0),"capacidad")}${kpi("Industria",fmt1(region.industry||0),"índice")}${kpi("Energía",fmt1(region.energy||0),"índice")}${kpi("Slots",`${used}/${region.capacitySlots||0}`,"ocupados")}${kpi("Control",`${controller?.flag||"🏳️"} ${controller?.name||c.name}`,owned?"Propio":"Ocupado")}${kpi("Estabilidad",pct(region.stability||0),"social")}${kpi("Defensa",fmt1(region.defense||45),"territorial")}</div>
      ${region.reconstruction&&region.reconstruction.status!=="complete"?`<section class="card reconstruction-card"><div class="card-title"><h3>🏗️ Reconstrucción de ${esc(region.name)}</h3><span>Daño pendiente ${fmt1(region.reconstruction.damage)}%</span></div><p>Recupera infraestructuras y reabre instalaciones para incorporar la región al tejido productivo nacional.</p><div class="reconstruction-actions"><button data-action="reconstructRegion" data-region-id="${region.id}" data-scope="infrastructure">Infraestructura</button><button data-action="reconstructRegion" data-region-id="${region.id}" data-scope="industry">Industria</button><button data-action="reconstructRegion" data-region-id="${region.id}" data-scope="energy">Energía</button><button class="primary-btn" data-action="reconstructRegion" data-region-id="${region.id}" data-scope="all">Plan integral</button></div></section>`:""}
  ...
  function renderStock(){
    ...
    const owned=all.filter(x=>(E().getHolding?.(state,x.id,c.id)||0)>0),portfolio=all.reduce((sum,x)=>sum+(x.marketCap||0)*(E().getHolding?.(state,x.id,c.id)||0)/100,0),controlledCompanies=all.filter(x=>(E().getHolding?.(state,x.id,c.id)||0)>=51).length;
    return `${heading("Bolsa y participaciones estratégicas","Empresas reales como referencia; precios y estados financieros completamente simulados",`<span class="simulation-badge">DATOS FICTICIOS · NO EN VIVO</span>`)}
      ...
      <section class="card owned-holdings"><div class="card-title"><h3>⭐ Mis participaciones</h3><span>${owned.length} empresas localizadas</span></div>${owned.length?`<div class="company-grid stock-grid">${owned.map(companyCard).join("")}</div>`:`<div class="empty-state"><span>⭐</span><p>Aún no tienes participaciones. Las empresas compradas aparecerán aquí.</p></div>`}</section>
  ...
  function renderTechnology() {
    ...
    return `${heading("Investigación y soberanía tecnológica",`${C().technologies.length} tecnologías en ${branches.length} ramas`)}
      <div class="kpi-grid four">${kpi("Puntos I+D",fmt0(c.researchPoints),"disponibles")}${kpi("Generación prevista",`${fmt1(c.researchMonthlyRate||0)}/mes`,`Inversión I+D ${fmt1(c.budgets.research)}%`)}${kpi("Completadas",fmt0(c.completedTechs.length),"tecnologías")}${kpi("En curso",fmt0(c.techQueue.length),"proyectos")}${kpi("Nivel nacional",fmt1(c.systems.technology),"capacidad")}</div>
  ...
  function renderDiplomacy() {
    ...
    let others=state.countries.filter(x=>x.id!==c.id&&x.sovereign!==false);
    ...
    return `${heading("Diplomacia mundial",`${state.countries.filter(x=>x.sovereign!==false).length} Estados soberanos; los países anexionados forman parte del vencedor`, `<input id="diplomacySearch" value="${esc(diplomacyQuery)}" placeholder="Buscar país…">`)}
      ...
      <section class="card"><div class="card-title"><h3>🚢🚚 Corredores comerciales</h3><span>Rutas marítimas y terrestres asignadas por geografía</span></div><div class="trade-route-grid">${routes.length?routes.map(r=>tradeRouteCard(r,c)).join(""):`<p class="muted">Firma un acuerdo comercial para activar rutas de buques o camiones.</p>`}</div></section>
  ...
  function importantEvents(){const allowed=E().importantEventTypes||new Set(["system","military","battle","politics","decision","objective","region","intelligence"]);return (state.events||[]).filter(e=>allowed.has(e.type)&&!/buque|camión|llegada|carga|corredor comercial|comercio amplían/i.test(`${e.title} ${e.text}`))}
  function renderMiniEvents(){const box=document.getElementById("miniEvents");if(!box)return;box.innerHTML=importantEvents().slice(0,7).map(e=>`<div class="mini-event"><span>${eventIcon(e.type)}</span><p>${esc(e.title)}</p><time>${e.date.slice(5)}</time></div>`).join("")}
  ...
  function facilityCard(f){const d=buildingDef(f.typeId)||{},cost=E().upgradeCost?.(f)||0;return `<article class="building-card"><header><h3>${d.icon||"🏢"} ${esc(d.name||f.typeId)}</h3><b>N${f.level||1}</b></header><p>${esc(d.capacity||d.description||"")} · ${fmt0((d.jobs||0)*(f.level||1))} empleos</p><footer><button data-action="upgradeBuilding" data-building-id="${f.id}" ${(f.level||1)>=(d.maxLevel||4)?"disabled":""}>${(f.level||1)>=(d.maxLevel||4)?"Nivel máximo":`Ampliar · ${money(cost)}`}</button></footer></article>`}
  ...
  function techCard(t,c){const done=c.completedTechs.includes(t.id),active=c.techQueue.find(q=>q.techId===t.id),locked=(t.requires||[]).some(x=>!c.completedTechs.includes(x)),unlocks=E().technologyUnlocks?.[t.id]||[];return `<article class="tech-card ${locked?"locked":""}"><header><h3>${t.icon} ${esc(t.name)}</h3><b>${done?"✓":t.cost}</b></header><p>${esc(t.description)}</p><div class="tech-unlocks"><span>🏭 Desbloquea / potencia</span><b>${esc(unlocks.length?unlocks.join(" · "):`Industria de ${t.branch}`)}</b></div><div class="info-row"><span>Duración</span><b>${t.months} meses</b></div>${active?progress(active.totalMonths||t.months,active.monthsRemaining):done?`<b class="positive">COMPLETADA</b>`:`<button data-action="research" data-tech-id="${t.id}" ${locked?"disabled":""}>Investigar</button>`}</article>`}
  ...
  function tradeRouteCard(route,c){const otherId=route.countries.find(id=>id!==c.id),other=E().getCountry(state,otherId),ships=route.ships||[],lead=ships[0],cargo=lead?.cargo||{},icon=route.transportMode==="land"?"🚚":"🚢";return `<article class="trade-route-card"><header><div><b>${c.flag} ${esc(c.name)} ⇄ ${other?.flag||"🏳️"} ${esc(other?.name||otherId)}</b><small>${route.transportMode==="land"?"Corredor terrestre":"Ruta marítima"}${route.spanishPortName?` · entrada por ${esc(route.spanishPortName)}`:""} · eficiencia ${fmt1(route.efficiency||0)}%</small></div><strong>${ships.length} ${icon}</strong></header><div class="trade-cargo"><span>${cargo.icon||"📦"}</span><div><b>${esc(cargo.name||"Carga mixta")}</b><small>${fmt1(cargo.quantity||0)} ${esc(cargo.unit||"")} · ${lead?.status||"En ruta"}</small></div></div><div class="meter"><i style="width:${clamp((lead?.progress||0)*100,0,100)}%"></i></div></article>`}
